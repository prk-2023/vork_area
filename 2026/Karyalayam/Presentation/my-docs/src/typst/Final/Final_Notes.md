...
Since our team has already covered eBPF in depth during previous technical sharing sessions 
And many of us are interacting with tools like BCC, bpftrace, and traditional libbpf for our daily work
I will skip re-explaining the core runtime model from scratch.
We all know how the subsystem behaves.

However, using Rust, there is one foundational rule we must establish for the rest of this presentation: 

=> Rust does not change the eBPF execution model. 

The Linux kernel verifier, the In-Kernel Just-In-Time (JIT) compiler, and the event-driven hook
architecture remain completely unchanged. 

The kernel execution engine only consumes raw machine bytecode. 
Whether that specific sequence of BPF instructions was emitted by clang processing a C file, or by rustc 
evaluating a Rust crate, makes absolutely zero difference to the verifier or the execution path. 

If the verifier accepts the program, it runs with zero overhead, securely in the kernel.
...
- eBPF is linux technology which allows to run sandboxed programs directly in kernel. 
- This evolved from fast packet filter BPF framework and extended to general purpose execution engine. 
- This loaded programs allow custom kernel behaviour with out kernel source modification.

- The loaded programs are first verified by the kernel for safety, before Execution 
// ( allowing only bounded loops, valid memoryaccesss...)
- Once the the programs gets verified  kernel compiles the byte code using a JIT compiler.
- The generated byte code is attached by the user space loader to desired kernel hook points. 
- Once these events related to the hook points are triggerd the program executes. 

These entire process can be grouped into 4 steps:

- We still step through 
    writing source, 
    verifying bounds, 
    JIT-compiling to native machine code, and 
    attaching to deterministic hook points.

- Hook point are predefined locations in kernel where these programs are attached. 
  The table shows some example hook points. 

  // kprobes for arbitrary kernel function boundaries, stable tracepoints for tracing, XDP for high-performance packet processing directly in the NIC driver ring buffers, and LSM for security policy enforcement.
...
- Maps act as shared memory between your kernel BPF code and the user-space reader.
- They are created by the loader tool before the BPF program itself is attached.
- Both kernel and user  space access them using standard file descriptors.

Our common map types and their typical use cases include:
- HASH:  for standard key-to-value lookups.
- RINGBUF: for high-throughput event streams.
- PERCPU_ARRAY: for lock-free, per-CPU data statistics.


Because eBPF maps are built directly out of native kernel memory structures, our programs read and write 
to them using highly optimized kernel helper functions like `bpf_map_lookup_elem` and `bpf_map_update_elem`.


The critical point relevant to the Rust sections ahead is the ring buffer (BPF_MAP_TYPE_RINGBUF), 
which was introduced in Linux 5.8. 

- Ring buffers have become gold standard for streaming data out of the kernel to user space because:

   1. It supports variable-length records, eliminating fixed-size payload constraints and overhead.
   2. It uses a single contiguous memory allocation, which is cache-friendly.
   3. It is fully compatible with standard Linux asynchronous I/O (epoll). 
      Inside Aya, this maps natively to `AsyncFd`, allowing it to directly drive our `Tokio async loops`
      without manual polling overhead.

The code example shows how maps are defined in Aya. Which is a crate level map definition:
```#[map]
static EVENTS: RingBuf = RingBuf::with_byte_size(4 * 1024 * 1024, 0);

```
This sets up a 4 MB ring buffer directly in the kernel, ready to interface with our asynchronous user-space loop.
...
From its inception to now there are 3 generations of eBPF Development 

- **Generation 1 (BCC/bpftrace):** 
    BCC: This model requires full Clang/LLVM infrastructure on the target at runtime/compiling.
    bpftrace: provides DTrace-style one-liners, ideal for quick kernel investigation.
    
    Both of these depend on LLVM and full kernel headers. Which offers challenge to deploy on embedded
    targets. 

- **Generation 2 (libbpf + CO-RE):** 
    This model allows us to compile BPF program ahead-of-time using Clang, embedding the BTF type 
    information inside the resulting ELF file. 
    It runs on any kernel configured with CONFIG_DEBUG_INFO_BTF=y. 
    Compile once, ship a small pre-compiled object and `libbpf.so`. (This was covered in previous talk)

    Deployment require libbpf.so on the target.

- **Generation 3 (language-native frameworks):** 
    Generation 3 is for language-native frameworks: (Rust / Go ) As these languages bring native type safety, 
    modern package managers, built-in asynchronous programming features.


Aya (pure Rust), cilium/ebpf (pure Go), libbpf-rs (Rust
  bindings over libbpf). First-class language integration: type system, package managers, native async,
  single binary.
  - For Rust: we have Aya: Written entirely in pure Rust, removing the libbpf dependency and the entire
            C compiler toolchain at runtime.

  - cilium/ebpf (Pure Go): Written in pure Go, though the BPF kernel-space code must still be compiled 
                           separately with Clang and packaged via bpf2go

  - libbpf-rs (Rust Bindings): This provides idiomatic Rust bindings wrapped over the standard C libbpf
                               runtime library, rather than a full language reimplementation.

Our focus main focus is **Aya**: Its the only framework that gives you one language, one toolchain, and 
zero C runtime dependencies from kernel code to userspace loader.
...
The take away of this slide is :
...
The tale shows Production Aya deployments today: 
...
Typically eBPF development for writing programs spans different HW designs running OWRT, Yocto, Android. 

But These devices run different Linux kernel versions, and Each customer might also add their own kernel 
patches.


Since in Linux kernel source code, structures change between versions. As shows with task_struct->pid. 

For this constrain we had to recompile eBPF programs foe ever target. 

CO-RE solves this problem completely:

- The Basic Idea of Core is, the compiler creates a bytecode and adds a special "relocation records" inside
a section called `.BTF` in the ELF file.  Which records which structure field the code wants to read. 

- At load time the program reads /sys/kernel/btf/vmlinux and fills these relocation records with correct 
  values. This works are as long as the target system supports CONFIG_DEBUG_INFO_BTF. 

=> Both `libbpf` and `Aya` automatically adjusts the memory offsets in the bytecode to match the running kernel
  exactly.

The difference between libbpf and Ays is the  **Target dependencies:** 
...
The standard libbpf application pipeline consists of five key phases:
- Write: write bpf c using kernel types exports (vmlinux.h) and helper definitions and standard SEC() macro.
- Compile: Invoking clang to generate BPF obj file (containing the ELF structures and BTF debug definitions)
- Skeleton: Using bpftool to generate the skeleton header (program.skel.h), which provides typed
            C structures for our user-space loader.
- Load + Verify: Running the host code through `skel__open()` and `skel__load()` to resolve CO-RE relocations 
                 and pass bytecode to the kernel verifier.
- Attach + Run: Invoking `skel__attach()` to return a `bpf_link` file descriptor, execute on specific 
                hook points, and read from active maps.

The steps are outlined in the slide.

Deploying: 
When deploying to target hardware, the runtime environment must host:
    libbpf.so (unless we explicitly configure static linking with libbpf.a).
    libelf.so, which is structurally required by libbpf to parse ELF objects.
    libz.so, which is required by libelf for internal decompression tasks.
    The standalone program.bpf.o object file itself, if it wasn't statically embedded via the skeleton 
    generation step.
...
Rust's `#![no_std]` mode is the natural fit for the eBPF kernel side :
    - no standard library, 
    - no allocator, 
    - just core primitives. 


`aya-ebpf` crate provides the BPF helpers, map types, and program macros for this restricted environment.

Type safety across the kernel boundary: 

This is  **The highest-value safety property:**

The most common eBPF bug class in C is a struct layout divergence between the kernel program and the 
userspace loader. 

The code examples show how a event struct defined in kernel and userspace program can diverge leading to
...
As These two definitions live in separate files. One in userspace and one in ebpf code. 
...
- one `#![no_std]` **common crate**, compiled into both sides.

- The struct is defined once. ( in the common crate )

- And if there are any Layout disagreement leads to **compile error, not a runtime bug.**
...
Let us trace how bytecode generation works under the traditional C workflow on the left.
- start with kernel space file: `program.bpf.c`
- compiles using: `→ clang -target bpf -O2 -g` generate 
- intermediate object, program.bpf.o, which contains the raw BPF bytecode along with the necessary 
  BTF debug sections.
- pass that ELF object to `bpftool gen skeleton` to generate our C loader header file, `program.skel.h.`
- Finally, we feed that generated header to gcc or clang, to compile the user-space file 

This Requires: clang, LLVM, bpftool, libelf, libbpf. Even if your userspace is in Rust (`libbpf-rs`), 
you still need the full C toolchain for the BPF kernel code.

The Right side of the slide is the Aya pipeline:
- Aya organizes your code natively into a single Cargo workspace split into three distinct crates:
    - `program-ebpf/src/main.rs` handles our kernel-space eBPF logic.
    - `program-common/src/lib.rs` stores our shared structures and type.
    - `program/src/main.rs` operates as our user-space side drive.
- To compile the kernel side, it uses `rustc --target bpfel-unknown-none` driven by a specialized linker 
   called `bpf-linker`, which utilizes the LLVM BPF backend under the hood.
- This produces an ELF object that is immediately embedded directly inside our user-space code at build time.
  //using the standard include_bytes_aligned! macro.
- From here, a simple `cargo build` (host) command wraps everything up into a single, completely 
  self-contained user-space program binary.

This requires: `rustc` (nightly) and the `bpf-linker` plugin. 
       There is absolutely no `clang`, no `bpftool`, no `libelf`, and no C toolchain required in your build
       path. 
- The BPF ELF is embedded directly into the user-space application. Which is  a single deployable binary on
  the target. 
...
Unlike libbpf, which is a monolithic C library, Aya is built as a highly modular family of specialized Rust 
crates.

Looking at the execution stack on the left:

-  User-Space Binary (std, async): This is your main application driver. It invokes `Ebpf::load()` to read 
        the bytecode, manages map access, and holds onto the program link handles.

- aya (Core User-Space Library): This acts as our pure Rust `bpf()` syscall wrapper. 
  It defines and exposes program types like `KProbe`, `KRetProbe`, Xdp, along with map types 
  //(HashMap, RingBuf, Array, and PerfMap)

- aya-obj (ELF + BTF + CO-RE Engine): This is a pure Rust ELF parser. 
        It reads the host target's /sys/kernel/btf/vmlinux file and patches CO-RE relocations directly in 
        memory, entirely removing any dependency on an external libelf library.

- aya-log (User-Space Log Receiver): This component reads raw logging structures out of the dedicated 
    `aya-log-ebpf` ring buffer.

Next we have 

- BPF Program (no_std, no_main): Which  is the kernel-space execution code utilizing attributes like
    `#[kprobe]`, `#[xdp]`, and `#[lsm]` to interact with maps.

- `aya-ebpf` (BPF-Side Runtime Crate): 
    This provides our core:
        - kernel-space macros(#[kprobe], #[map], #[xdp]), 
        - helper wrappers such as `bpf_ktime_get_ns()`, and 
        - native BPF map structures like HashMap, RingBuf, and PerfMap.

- `aya-log-ebpf`: Handles BPF-side logging using info!() and warn!() macros that pipe events straight into 
   the log ring buffer.

// - `aya-ebpf-bindings`: 
//     Provides the kernel `uapi` types, which are auto-generated directly from kernel headers via bindgen.

The table on the right shows the precise 1:1 architectural replacements that occur when transitioning away 
from the libbpf ecosystem: 
