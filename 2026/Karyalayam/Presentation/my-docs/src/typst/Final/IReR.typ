...
#cols[
  ...
  *Hook types that are used* ( event driven callback sites )
...
#cols[
  ...
  *`no_std` alignment*
  ...
  - eBPF programs run with no kernel library, no allocator, no OS primitives. 
  - Rust's `#![no_std]` mode is the natural target. 
  - `aya-ebpf` provides the BPF-side runtime (`bpf_helpers`, map types, program macros) without any standard library dependency.
...
== Three-crate workspace structure (using aya-template)
...
= How Rust Improves the eBPF Development Experience
...
#callout(color: ebpf-teal)[
  - One simple program demonstrating the full Aya stack: BPF kernel code in Rust, userspace loader in Rust, shared types, logging, async integration. 
  - The same patterns scale to production tools like bpfman, ebpfguard, and Blixt.
]
// ─────────────────────────────────────────────────────────────────────────────
//  CLOSING FOCUS SLIDE
// ─────────────────────────────────────────────────────────────────────────────
