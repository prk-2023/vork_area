# trace_pipe5: output to file:

## Feature **Output to File**

### What it does:

Adds support for logging trace output to a file in addition to printing it on screen.
...
```bash
./trace_pipe sched sched_switch --out trace.log
```

Output:

* All trace events are printed to `stdout` (as before)
* Also written to `trace.log`
...
* Add a new optional argument: `--out <filename>`
* Open the file in append mode (or create if missing)
* Write every captured trace line into it

---

### Updated `main.rs` with Output File Support
...
--> Creates `trace.log` if it doesn’t exist
--> Appends all trace output to the file
--> Still shows output on terminal
